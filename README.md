...
This repository contains a Vue 3 single-page application (SPA) for the classifier service.
...
**Note**: This template uses `@agile-crafts-people/impact_spa_utils` for reusable components, composables, and utilities. See the [spa_utils README](../spa_utils/README.md) for complete documentation on available components (`AutoSaveField`, `AutoSaveSelect`, `ListPageSearch`), composables (`useResourceList`, `useErrorHandler`, `useRoles`), and utilities (`formatDate`, `validationRules`).
...
This template uses components and composables from `@aagile-crafts-people/impact_spa_utils`:
