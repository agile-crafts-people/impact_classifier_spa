describe('Comment Domain', () => {
  ...
  it('should search for comments using existing test data', () => {
    cy.visit('/comments')
    ...
    // Check if table has data, if so get first comment name to search for
    cy.get('body').then(($body) => {
      const hasRows = $body.find('table tbody tr').length > 0
      
      if (hasRows) {
        // Get the first comment name from the table to search for
        cy.get('table tbody tr').first().find('td').first().invoke('text').then((firstItemName) => {
          ...
          if (trimmedName.length > 0) {
            ...
            // Search for comments using partial name
            cy.get('[data-automation-id="comment-list-search"]').find('input').type(searchTerm)
            // Wait for debounce (300ms) plus API call
            cy.wait(800)
            
            // Verify the search results contain the comment (should find at least the one we searched for)
        ...
        cy.get('[data-automation-id="comment-list-search"]').find('input').should('exist')
        cy.get('[data-automation-id="comment-list-search"]').find('input').type('test')
        cy.wait(800)
        cy.get('[data-automation-id="comment-list-search"]').find('input').clear()
      }
    })
  ...
  it('should search for comments and filter results', () => {
    cy.visit('/comments')
    ...
    cy.get('table tbody tr').then(($rows) => {
      ...
      cy.get('[data-automation-id="comment-list-search"]').find('input').type('nonexistent-search-term-xyz')
      // Wait for debounce (300ms) plus API call
      cy.wait(800)
  ...
  it('should not have a new comment button (read-only)', () => {
    cy.visit('/comments')
