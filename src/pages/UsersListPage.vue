    ...
    <v-row>
      <v-col cols="12">
        <ListPageSearch
          ...
          automation-id="user-list-search"
        />
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(users ?? []) as unknown as User[]"
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="user-list-load-more"
            >
...
/**
 * Users List Page - Showcase of impact_spa_utils simplicity
 * 
...
const {
  ...
  queryKey: ['users'],
  queryFn: (params) => api.getUsers(params),
  getItemId: (item) => item._id,
  limit: 20,
})

function navigateToUser(_event: unknown, { item }: { item: User }) {
