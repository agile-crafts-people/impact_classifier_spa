<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">comments</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12">
        <ListPageSearch
          ...
          automation-id="comment-list-search"
        />
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(comments ?? []) as unknown as comment[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateTocomment"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="comment-list-load-more"
            >
...
/**
 * comments List Page - Showcase of impact_spa_utils simplicity
 * 
...
import { ListPageSearch, useInfiniteScroll } from '@agile-crafts-people/impact_spa_utils'
...
const {
  ...
  queryKey: ['comments'],
  queryFn: (params) => api.getcomments(params),
  getItemId: (item) => item._id,
  limit: 20,
})

function navigateTocomment(_event: unknown, { item }: { item: comment }) {
  router.push(`/comments/${item._id}`)
}
