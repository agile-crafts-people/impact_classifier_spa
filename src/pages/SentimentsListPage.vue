<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Sentiments</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12" md="6">
        <ListPageSearch
          ...
          automation-id="sentiment-list-search"
        />
      </v-col>
      <v-col cols="12" md="6" class="d-flex justify-end align-center">
        <v-btn 
          color="primary" 
          to="/sentiments/new"
          data-automation-id="sentiment-list-new-button"
        >
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(sentiments ?? []) as unknown as Sentiment[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToSentiment"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="sentiment-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load sentiments: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * Sentiments List Page - Showcase of impact_spa_utils ease of use
 * 
 ...
 * from @agile-crafts-people/impact_spa_utils. Just provide your API call and you're done!
 */
import { computed } from 'vue'
import { api } from '@/api/client'
import { formatDate, ListPageSearch, useInfiniteScroll } from '@agile-crafts-people/impact_spa_utils'
...
const {
  ...
  setSortBy,
  setOrder,
} = useInfiniteScroll<Sentiment>({
  queryKey: ['sentiments'],
  queryFn: (params) => api.getSentiments(params),
  getItemId: (item) => item._id,
  limit: 20,
})

// Simple navigation handler
function navigateToSentiment(_event: unknown, { item }: { item: Sentiment }) {
  router.push(`/sentiments/${item._id}`)
}
