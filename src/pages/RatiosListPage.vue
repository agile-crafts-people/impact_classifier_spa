<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Ratios</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12" md="6">
        <ListPageSearch
          ...
          automation-id="ratio-list-search"
        />
      </v-col>
      <v-col cols="12" md="6" class="d-flex justify-end align-center">
        <v-btn 
          color="primary" 
          to="/ratios/new"
          data-automation-id="ratio-list-new-button"
        >
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(ratios ?? []) as unknown as Ratio[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToRatio"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="ratio-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load ratios: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * Ratios List Page - Showcase of impact_spa_utils ease of use
 * 
...
const {
  ...
  setSortBy,
  setOrder,
} = useInfiniteScroll<Ratio>({
  queryKey: ['ratios'],
  queryFn: (params) => api.getRatios(params),
  getItemId: (item) => item._id,
  limit: 20,
})

// Simple navigation handler
function navigateToRatio(_event: unknown, { item }: { item: Ratio }) {
  router.push(`/ratios/${item._id}`)
}
