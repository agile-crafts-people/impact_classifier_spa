<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">ratioss</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12" md="6">
        <ListPageSearch
          ...
          automation-id="ratios-list-search"
        />
      </v-col>
      <v-col cols="12" md="6" class="d-flex justify-end align-center">
        <v-btn 
          color="primary" 
          to="/ratioss/new"
          data-automation-id="ratios-list-new-button"
        >
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(ratioss ?? []) as unknown as ratios[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToratios"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="ratios-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load ratioss: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * ratioss List Page - Showcase of impact_spa_utils ease of use
 * 
...
const {
  ...
  setSortBy,
  setOrder,
} = useInfiniteScroll<ratios>({
  queryKey: ['ratioss'],
  queryFn: (params) => api.getratioss(params),
  getItemId: (item) => item._id,
  limit: 20,
})

// Simple navigation handler
function navigateToratios(_event: unknown, { item }: { item: ratios }) {
  router.push(`/ratioss/${item._id}`)
}
