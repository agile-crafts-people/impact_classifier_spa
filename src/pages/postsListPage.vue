    ...
    <v-row>
      <v-col cols="12">
        <ListPageSearch
          ...
          automation-id="post-list-search"
        />
      </v-col>
    </v-row>
        ...
        <v-card>
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="post-list-load-more"
            >
...
/**
 * posts List Page - Showcase of impact_spa_utils simplicity
 * 
...
const {
  ...
  queryKey: ['posts'],
  queryFn: (params) => api.getposts(params),
  getItemId: (item) => item._id,
  limit: 20,
})

function navigateTopost(_event: unknown, { item }: { item: post }) {
